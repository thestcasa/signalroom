...
def write_html_report(bundle: dict[str, object], output: Path) -> None:
    ...
    routines = "".join(
        f"<tr><td>{escape(str(row['routine']))}</td><td>{row['count']}</td><td>{float(row['share']) * 100:.1f}%</td><td>{row['shots']}</td><td>{float(row['xg']):.2f}</td></tr>"
    ...
    <table><thead><tr><th>Routine</th><th>n</th><th>Share</th><th>Shots</th><th>xG</th></tr></thead><tbody>{routines}</tbody></table></section>
    ...
    <footer><img src="assets/statsbomb-open-data-logo.png" alt="StatsBomb" class="source-logo"><br><b>Historical analysis, not current tactical advice.</b> Data: StatsBomb Open Data. No club affiliation or endorsement is claimed. Generated {escape(str(bundle["generated_at"]))}.</footer>
    </main></body></html>"""
...
def _css() -> str:
    return """
    :root{--ink:#102a43;--teal:#00a6a6;--coral:#ff6b5e;--paper:#f7f5ef;--mist:#e8f1f5}*{box-sizing:border-box}
